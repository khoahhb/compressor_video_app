    ...
    private void doEncodeDecodeVideoFromBuffer(MediaCodec encoder, int encoderColorFormat,
                ...
                if (inputBufIndex >= 0) {
                    ...
                    if (generateIndex == NUM_FRAMES) {
                        ...
                        if(inputBuf.capacity() >= frameData.length){
                            ...
                            Log.e(TAG, "inputBuf.capacity smaller than frameData");
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (outputStream != null) {
                        ...
                        try {
                            ...
                            Log.e(TAG, "failed writing debug data to file");
                            throw new RuntimeException(ioe);
                        }
                    }
        ...
        if (outputStream != null) {
            try {
                ...
                Log.e(TAG, "failed closing debug file");
                throw new RuntimeException(ioe);
            }
        }
        if (outputSurface != null) {
        ...
        if(mWidth == cropWidth && mHeight == cropHeight){
            for (int i = 0; i < 8; i++) {
                ...
                if (!isColorClose(testY, expY) ||
                    ...
                    Log.e(TAG, "Bad frame " + frameIndex + " (rect=" + i + ": yuv=" + testY +
    ...
    private boolean checkSurfaceFrame(int frameIndex) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (!isColorClose(r, expR) ||
                    !isColorClose(g, expG) ||
                    !isColorClose(b, expB)) {
                Log.e(TAG, "Bad frame " + frameIndex + " (rect=" + i + ": rgb=" + r +
